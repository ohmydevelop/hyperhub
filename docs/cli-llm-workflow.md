...
每条请求都有 RFC 4122 UUID，并展示当前 `n/m` 进度、“新增 / 修改 / 删除”动作、与 Config TUI 相同的分类路径和字段名称，以及脱敏变更。原始 JSON Patch 的 `op/path` 仅放在 `technical` 区域，避免成为主要认知界面。可选择：
...
3. 逐条展示唯一 UUID、`n/m` 进度和新增/修改/删除语义；
4. 审批分类路径与 Config TUI 使用同一语义定义；
5. 单条请求可以拒绝、编辑或批准；
6. `${APPROVE:name}` 通过星号掩码输入填写且不会泄漏；
7. 中断后只继续未处理请求，UUID 保持不变；
8. 已批准请求立即持久化，完成后删除队列；
9. 保存后的配置可通过 `validate`；
10. Serve 运行时批准请求可以热更新。
