...
- [ ] **工具网关**：统一管理 MCP 生命周期，将 MCP Tool 转换为 CLI 工具，无缝衔接 Shell 和图灵完备环境。
...
- [ ] 通过意图进行规则配置以及事件分析。
- [ ] 在 LLM 读取配置时配置安全栅栏，对敏感数据进行脱敏，但不影响配置。

## MCP 网关

- [ ] 让 MCP 具备 CLI 的通用性、可组合性、可隔离性，并与 Agent 解耦，而不是构建传统意义上的 HTTP 聚合网关。
- [ ] 避免将 MCP 工具组合逻辑全部交给 LLM 推理。
- [ ] 让 MCP 通过 CLI 接入图灵完备环境。
- [ ] 缓解上下文压力。
