...
`linux_probe.c` 用于验证动态 ELF 的默认 ptrace 后代覆盖和显式 Gum 完整 manifest：DNS、阻塞与非阻塞 connect、延迟 SOCKS 握手、send/recv、fcntl/ioctl、open/open64/openat/openat64/creat、read/write、rename/unlink 变体、mmap/mprotect/munmap、execve 失败路径，以及 fork+exec、posix_spawn/posix_spawnp 后代递归注入。
...
`linux_http_credential_probe.py` 是动态 ptrace 的 HTTP 路由回归夹具：客户端以 `localhost` 建连并发送占位 Authorization，服务端只接受经协议探测重新匹配 URL 路由后注入的 Bearer 值。它覆盖系统 DNS 无可观测报文时，IP-form SOCKS 连接依据 HTTP Host/path 精化策略的路径。
